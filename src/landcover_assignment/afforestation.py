...
class Afforestation:
    ...
    def compute_cbm_afforestation(
        self, sc,future_forest_area, cbm_dataframe
    ):
        cbm_df = self.cbm_dataframe_structure()

        scenario_data = self.data_manager_class.scenario_inputs_df

        grouped_data = scenario_data.drop_duplicates(subset=["Scenarios", "Conifer proportion"]).reset_index(drop=True)
    ...
    def get_afforest_areas(self, transition_matrix):
        ...
        for i in transition_matrix.index:
            ...
            afforest_df.at[index, "area_ha"] = transition_matrix.at[i ,"Grassland_to_Forest"]
