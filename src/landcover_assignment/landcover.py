...
class LandCover:
    ...
    def compute_current_area(self):
        ...
        calculated_current_areas_pd = pd.DataFrame(
            columns=self.data_manager_class.areas_dataframe_cols
        )

        calculated_current_areas_pd[
            "land_use"
        ] = self.data_manager_class.land_use_columns
        calculated_current_areas_pd["farm_id"] = -calibration_year
        ...
        for index in calculated_current_areas_pd.index:
            ...
            if land_use == "farmable_condition":
                ...
                if method:
                    ...
                    estimated_area = self.total_grassland.loc[
                        calibration_year, 0
                    ].item()
                    area_value = df.loc[calibration_year, "total_kha"].item()
                    share_mineral = (
                        df.loc[calibration_year, "mineral_kha"].item() / area_value
                    )
                    share_organic = 1 - share_mineral
                    share_burnt = (
                        df.loc[calibration_year, "burnt_kha"].item() / area_value
                    )
                ...
                if method:
                    ...
                    share_organic = (
                        df.loc[calibration_year, "organic_kha"].item() / area_value
                    )
                    share_burnt = (
                        df.loc[calibration_year, "burnt_kha"].item() / area_value
                    )
                ...
                if land_use == "wetland":
                    share_rewetted_in_organic = (
                        df.loc[calibration_year, "rewetted_organic_kha"].item()
                        / area_value
                    )
                    share_rewetted_in_mineral = (
                        df.loc[calibration_year, "rewetted_mineral_kha"].item()
                        / area_value
                    )
                    share_peat_extraction = (
                        df.loc[calibration_year, "peat_extraction_kha"].item()
                        + df.loc[calibration_year, "drained_extraction_kha"].item()
                    ) / area_value

                    calculated_current_areas_pd.loc[
                        index, "share_rewetted_in_organic"
                    ] = share_rewetted_in_organic
                    calculated_current_areas_pd.loc[
                        index, "share_rewetted_in_mineral"
                    ] = share_rewetted_in_mineral
                    calculated_current_areas_pd.loc[
            ...
            for landuse in land_use_columns:
                if landuse == "grassland":
                    ...
                    settlement_pd = current_area_pd.loc[
                        (current_area_pd["land_use"] == landuse)
                    ]
                    ...
                    temp = pd.DataFrame(
                        {
                            ...
                            "share_rewetted_in_mineral": settlement_pd[
                                "share_rewetted_in_mineral"
                            ].item(),
                            "share_peat_extraction": settlement_pd[
                                "share_peat_extraction"
                            ].item(),
    ...
    def spared_area_breakdown(self, scenario):
        ...
        scenario_subset = scenario_dataframe.loc[
            (scenario_dataframe["Scenarios"] == scenario)
        ]
        ...
        current_grassland_mask = (current_areas["year"] == calibration_year) & (
            current_areas["land_use"] == "grassland"
        )

        max_organic_available = min(
            initial_spared_area,
            (
                current_areas.loc[current_grassland_mask, "area_ha"].item()
                * current_areas.loc[current_grassland_mask, "share_organic"].item()
            ),
        )
        ...
        for land_use in spared_land_use_dict.keys():
            if land_use == "wetland":
                ...
                new_wetland_area = min(
                    max_organic_available,
                    (
                        initial_spared_area
                        * scenario_subset[spared_land_use_dict[land_use]].values[0]
                    ),
                )

                generated_land_use_data = LandDistribution.land_distribution(
                    land_use, current_areas, new_wetland_area
                )
                ...
                new_land_use_area = min(
                    adjusted_spared_area,
                    (
                        initial_spared_area
                        * scenario_subset[spared_land_use_dict[land_use]].values[0]
                    ),
                ...
                generated_land_use_data = LandDistribution.land_distribution(
                    land_use, current_areas, new_land_use_area
                )
                ...
                generated_land_use_data = LandDistribution.land_distribution(
                    land_use, current_areas, adjusted_spared_area
                )
                result_dict[land_use] = generated_land_use_data
    ...
    def grassland_breakdown(self, scenario):
        ...
        new_wetland_area = min(
            ...
            (
                initial_spared_area
                * scenario_subset[spared_land_use_dict["wetland"]].values[0]
            ),
        )
        ...
        generated_land_use_data = LandDistribution.grassland_distriubtion(
            current_areas, spared_mineral, spared_organic
        )
